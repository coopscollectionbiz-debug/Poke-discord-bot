...
if (isEvolve && isOwned) {
  ...
  if (!next) {
    ...
    // 🧩 Determine eligibility based on shiny mode
    let canEvolve = false;
    if (shinyMode) {
    ...
    if (canEvolve) {
      // ✅ Eligible — show cost badge and enable evolution
      ...
      // 🔒 Either not enough stones or missing correct variant
      addBadge(card, `🪨 ${cost}`, "evolve-cost");
      card.classList.add("unowned");
      addLock(card);
    }
...
// ===========================================================
// 💝 Donation Confirmation (Shiny-Aware)
// ===========================================================
...
if (shinyToggle) {
  shinyToggle.addEventListener("click", () => {
    ...
    shinyToggle.textContent = shinyMode
      ? "✨ Shiny Mode ON"
      : "Shiny Mode OFF";
  });
}
...
// ===========================================================
// 🎉 Evolution Popup (Shiny-Aware)
// ===========================================================
function showEvolutionPopup(fromName, toName, toId, isShiny = false) {
  const overlay = document.createElement("div");
  overlay.id = "evoPopupOverlay";

  // 🔹 Choose sprite path based on shiny
  const spritePath = isShiny
    ? `/public/sprites/pokemon/shiny/${toId}.gif`
    : `/public/sprites/pokemon/${toId}.gif`;
  ...
  popup.innerHTML = `
    <h2>${shinyText}${fromName} evolved into ${shinyText}${toName}!</h2>
